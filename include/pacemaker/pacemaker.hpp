...
namespace pacemaker {
	...
	struct JackClient {
		...
		std::function<int(jack_nframes_t)> callback;
		...
		JackPort port_register_output(const std::string& name) const {
			return { client, jack_port_register(client, name.c_str(), JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput, 0) };
		}

		JackPort port_register_input(const std::string& name) const {
		...
		bool set_callback(F&& fn) {
			...
			const auto wrapper = [](jack_nframes_t nframes, void* arg) {
				...
				return client.callback(nframes);
			};
		...
		std::vector<std::string> get_ports() const {
			return detail::null_array_to_vec<const char*, std::string>(
				jack_get_ports, client, nullptr, JACK_DEFAULT_MIDI_TYPE, 0);
		}

		std::vector<std::string> get_input_ports() const {
			return detail::null_array_to_vec<const char*, std::string>(
		...
		std::vector<std::string> search_ports(const std::string& port_name) const {
			return detail::null_array_to_vec<const char*, std::string>(
				jack_get_ports, client, port_name.c_str(), JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput);
		}
	};
